#ifndef CAFFE_Independent_LOSS_LAYER_HPP_
#define CAFFE_Independent_LOSS_LAYER_HPP_
...
/**
 * @brief Computes the Independent (L2) loss @f$
 ...
 *      the computed Independent loss: @f$ E =
 *          \frac{1}{2n} \sum\limits_{n=1}^N \left| \left| \hat{y}_n - y_n
 *        \right| \right|_2^2 @f$
 *
 * This can be used for least-squares regression tasks.  An InnerProductLayer
 * input to a IndependentLossLayer exactly formulates a linear least squares
...
class IndependentLossLayer : public LossLayer<Dtype> {
 public:
  explicit IndependentLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "IndependentLoss"; }
  /**
   * Unlike most loss layers, in the IndependentLossLayer we can backpropagate
...
//   virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  /**
   * @brief Computes the Independent error gradient w.r.t. the inputs.
   *
   * Unlike other children of LossLayer, IndependentLossLayer \b can compute
...
#endif  // CAFFE_Independent_LOSS_LAYER_HPP_
