function Dh=hammingDist(B1, B2, param)
%
...
if param.use_tree
    fprintf('computing tree-like hamming distance\n');
    % Dh_sp = zeros([n1 n2], 'uint16');
    % Dh_usp = zeros([n1 n2], 'uint16');

    sp_bits = param.sp_bits;
    usp_bits = param.nbits - sp_bits;
    tic
    % Dh_sp = B1(:, 1:sp_bits) * B2(:, 1:sp_bits)';
    % Dh = bitxor(B1, B2);
    Dh_sp = pdist2(B1(:, 1:sp_bits), B2(:, 1:sp_bits), 'hamming');
    toc
    tic
    % Dh_usp = B1(:, sp_bits+1:end) * B2(:, sp_bits+1:end)';
    Dh_usp = pdist2(B1(:, sp_bits+1:end), B2(:, sp_bits+1:end), 'hamming');
    toc
    Dh = Dh_sp * 10 + Dh_usp;
else
    tic
